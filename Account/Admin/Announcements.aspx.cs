...
public partial class student_Subject : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        SqlCommand cmd = new SqlCommand("insert into AnnouncementThread values('" + EMPID + "','" + txtDF.Text + "','" + Session["User"].ToString() + "','" + DateTime.Now.ToString() + "')", con);
        cmd.ExecuteNonQuery();
    ...
    protected void Order_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Submit")
        {
            ...
            if (Reply != "" || ReplyImage.ToString() != "")
            {
                Guid EMPID = Guid.NewGuid();
                con.Open();
                SqlDataAdapter adp = new SqlDataAdapter("insert into AnnouncementReply values('" + EMPID + "','" + e.CommandArgument + "','" + Reply + "','" + Session["User"].ToString() + "','" + DateTime.Now.ToString() + "')", con);
